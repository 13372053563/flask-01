...
from blue_prints.detect.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, \
    segments2boxes, \
    resample_segments, clean_str
from blue_prints.detect.utils.torch_utils import torch_distributed_zero_first
...
class LoadImages:  # for inference
    def __init__(self, path, old_shape, img_size=640, stride=32):
        images = [path]
        ni = len(images)
    ...
    def __next__(self):
        ...
        # Padded resize
        img = letterbox(img0, self.old_shape, self.img_size, stride=self.stride)[0]
...
def letterbox(img, old_shape, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True,
              stride=32):
    # Resize and pad image while meeting stride-multiple constraints
    shape = old_shape[:2]  # current shape [height, width]
